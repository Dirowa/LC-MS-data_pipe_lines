...
rtrmin <- min(rtime(raw_data))
rtrmax <- max(rtime(raw_data))
rtr <- c(rtrmin, rtrmax)
mzr <- c(0,10000000000)
...
sample_colors <- group_colors[chr_ex$class]
...
plot(chr_ex, col = group_colors[chr_raw$class], lwd = 2,
...
boxplot(ints, varwidth = TRUE, col = group_colors[xdata$class],
...
pdp <- PeakDensityParam(sampleGroups = xdata$class,
